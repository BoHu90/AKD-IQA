...
class DistillationIQASolver(object):
    def __init__(self, config):
        ...
        self.weights = nn.Parameter(torch.Tensor([3, 3, 0.8, 0.8]).cuda())
        print(f'weights:{self.weights.data}')
        ...
        for t in range(self.epochs):
            ...
            for LQ_patches, refHQ_patches, ref_patches, label in self.train_data:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    for i, (t_encode_diff_features, s_encode_diff_features) in \
                        ...
                        # feature_loss += self.weight[i] * kd_loss_single  # 每层各有权重
            ...
            if test_srcc + test_plcc + test_krcc > best_srcc + best_plcc + best_krcc:
                ...
                torch.save(self.studentNet.state_dict(), os.path.join(self.config.model_checkpoint_dir,
                                                                      f'Student_{config.train_dataset}{config.batch_size}_{config.lr}_in_saved_model.pth'))
                print("学生模型更新：")
...
if __name__ == "__main__":
    ...
    # 保存控制台输出
    # TODO 路径修改:日志
    saver = Saver(f'./MKD_logs/In/{config.train_dataset}_b{config.batch_size}_in.log', sys.stdout)
    sys.stdout = saver
